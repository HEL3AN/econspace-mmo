...
namespace
{
// Docking margin on the station radius; the same value StepPlayerDock enforces.
constexpr float ORDER_DOCK_RANGE = 90.0f;
...
void Simulation::StepPlayerOrder(SystemState& st, float dt)
{
    ...
    if (order_.kind == Orders::Kind::Dock)
        arrive = size + ORDER_DOCK_RANGE;
    else if (order_.kind == Orders::Kind::Mine)
        arrive = size + MINING_RANGE;
